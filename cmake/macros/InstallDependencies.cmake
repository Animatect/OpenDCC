...
if(WIN32)
    if(DCC_INSTALL_ADS)
        # For FetchContent-built qt5advanceddocking, get location from real target not alias
        if(TARGET qt5advanceddocking)
            get_target_property(_ads_aliased ads::qtadvanceddocking ALIASED_TARGET)
            if(_ads_aliased)
                # Use generator expression for FetchContent targets
                set(_ads_lib "$<TARGET_FILE:${_ads_aliased}>")
            else()
                set(_ads_lib "$<TARGET_FILE:ads::qtadvanceddocking>")
            endif()
        elseif(TARGET ads::qtadvanceddocking)
            # For pre-built library, try IMPORTED_LOCATION
            get_target_property(_ads_lib ads::qtadvanceddocking IMPORTED_LOCATION)
            if(NOT _ads_lib)
                get_target_property(_ads_lib ads::qtadvanceddocking IMPORTED_LOCATION_RELEASE)
            endif()
        endif()
        if(_ads_lib)
